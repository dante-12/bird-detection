# 使い方の詳しい説明
1. 準備
    * モザイク合成したい画像を、どこかのフォルダに入れる。以下、仮に`./numa/`とする。　　
        合成したくない画像はそのフォルダには入れないこと。
    * 撮影対象（地上・水面）の標高を確認。以下、仮に`-2.5m`とする。  
        コマンドの実行に必要なので何メートルか調べておく
...
4. 最初の実行でうまくいかなかったらGUIを起動して試行錯誤が必要
    ...
    * Reverse rotate - チェックすると上記の角度分だけ逆回転させる
    ...
    気に入ったところで`Save`ボタンを押して保存してください。ここでメモリー不足で強制終了してしまうことがありますが、その場合はなんとも。

    GUIに表示されているAltitudeやYaw offsetの数字を覚えておけば、次回以降はコマンドラインオプションからのそれらを直接指定し、GUIなしで保存まで行うことができるので便利です。

    <img src="attr_panel.png">

    この例だと`--lake-alt 5.7 --yaw-offset 0 --yaw-invert`となります。