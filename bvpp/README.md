...
* 撮影時にカメラがRoll（水平でなくなる）していないこと(Gimbal Roll Degreeが0かnullであること)
...
## 入力画像のゆがみ補正（実験的実装）
    * --undistort : ゆがみ補正を行う
    * --k1 0.1400 : ゆがみ補正用のk1パラメータ（デフォルト: 0.1400）
    * --k2 -0.3979 : ゆがみ補正用のk2パラメータ（デフォルト: -0.3979）
    * --k3 0.4837 : ゆがみ補正用のk3パラメータ（デフォルト: 0.4837）
