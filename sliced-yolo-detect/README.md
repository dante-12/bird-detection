...
4. 分析用のCSVファイルを出力する方法

    検出したオブジェクトの座標や信頼値などのデータをCSV形式のファイルに保存するには`--csv ファイル名`オプションをつけて実行してください。`yolo-label-reassembler`を使うにはここでCSVファイルを出力しておく必要があります。

    ```
    $ python sliced_yolo_detect.py ..略.. --csv target.csv
    ```

    このファイルには検出したオブジェクトのclass, 座標、大きさ、信頼度などデータが記録されます。
